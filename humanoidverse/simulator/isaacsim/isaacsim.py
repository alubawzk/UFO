...
class IsaacSim(BaseSimulator):
    ...
    @staticmethod
    def _resolve_asset_path(asset_root, asset_path):
        asset_path = str(asset_path)
        if asset_path.startswith("hf://"):
            try:
                from huggingface_hub import snapshot_download
            except ImportError as exc:
                raise ImportError(
                    "huggingface_hub is required to resolve hf:// robot assets."
                ) from exc

            asset_ref = asset_path[len("hf://"):]
            if "@" not in asset_ref:
                raise ValueError(f"HF asset URI must include a revision: {asset_path}")
            repo_id, revision_and_file = asset_ref.split("@", 1)
            if "/" not in revision_and_file:
                raise ValueError(f"HF asset URI must include a file path: {asset_path}")
            revision, filename = revision_and_file.split("/", 1)
            local_candidates = []
            if asset_root:
                asset_root_abs = os.path.abspath(str(asset_root))
                local_candidates = [
                    os.path.join(asset_root_abs, repo_id, revision, filename),
                    os.path.join(asset_root_abs, repo_id.split("/")[-1], revision, filename),
                    os.path.join(asset_root_abs, "g1", repo_id.split("/")[-1], revision, filename),
                ]
            for local_asset_path in local_candidates:
                if os.path.isfile(local_asset_path):
                    logger.info(f"Resolved HF robot asset from local mirror: {local_asset_path}")
                    return local_asset_path

            try:
                snapshot_dir = snapshot_download(repo_id=repo_id, revision=revision)
            except Exception as exc:
                checked_paths = ", ".join(local_candidates) if local_candidates else "none"
                raise RuntimeError(
                    f"Failed to resolve {asset_path}. HuggingFace download failed and local mirrors were missing: {checked_paths}"
                ) from exc
            return os.path.join(snapshot_dir, filename)

        if os.path.isabs(asset_path):
            return os.path.abspath(asset_path)
        return os.path.abspath(os.path.join(asset_root, asset_path))
    ...
    def _setup_scene(self):
        ...
        asset_abs_path = self._resolve_asset_path(asset_root, asset_path)

        if not os.path.isfile(asset_abs_path):
            raise FileNotFoundError(f"Robot asset not found: {asset_path} -> {asset_abs_path}")
        logger.info(f"Loading robot asset from {asset_abs_path}")
        ...
        if asset_abs_path.lower().endswith(".urdf"):
            spawn = sim_utils.UrdfFileCfg(
                ...
                self_collision=not bool(self.env_config.robot.asset.self_collisions),
        ...
        elif asset_abs_path.lower().endswith((".usd", ".usda")):
            ...
            raise ValueError(f"Unsupported robot asset type: {asset_abs_path}")
