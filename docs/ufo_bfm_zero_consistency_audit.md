...
控制频率两边均为 50 Hz；Mini3 使用 physics 500 Hz、decimation 10。
...
| Control delay | 默认关闭；预留 `[0,1]` physics step | 4340P 固定 4 步、脚踝随机 3–5 步、手臂 0 步；均按 500 Hz physics step 计数 | 默认和参数不一致 |
...
| Control-delay queue 清零 | 原始功能存在但默认 delay 关闭 | 是；脚踝重新采样 3–5 step，4340P 恢复固定 4 step | 结构一致、默认不同 |
